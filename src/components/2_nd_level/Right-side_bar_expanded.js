...
const RightSideBarExpanded = () => {
    return (
        ...
        ${styles.right_side_bar} ${styles.withMap}`}>
            ...
            <button className={`container-fluid col-12 p-2 fa fa-arrow-right d-flex flex-wrap justify-content-start m-0 pl-1`}>
                <p className={`pl-2`}> Collapse map </p>
            </button>

            <div id="map" className={`container-fluid col-11 p-2 ${styles.map}`}>Here must be google map</div>
        </div>
    );
};
